...
if ! _b=$(./target/release/trantor build "$FIX" --app app --out imview 2>&1); then echo "FAIL: build imview" >&2; echo "$_b" >&2; exit 1; fi
out=$("$FIX/target/trantor/imview-slice/bin/imview")
