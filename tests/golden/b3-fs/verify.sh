...
run() { # $1 world  $2 out  $3 expected escape line  $4 expected Path decl
  ./target/release/trantor compose "$B" --world "$1" >/dev/null
  local decl; decl=$(grep -m1 -E '^(Path|OsPath) :=' "$B/target/trantor/b3-fs/platform/Path.roc")
  [[ "$decl" == "$4"* ]] || { echo "FAIL [$1]: platform Path.roc decl is '$decl', want '$4'"; exit 1; }
  if ! _b=$(./target/release/trantor build "$B" --world "$1" --app app --out "$2" 2>&1); then echo "FAIL: build $2" >&2; echo "$_b" >&2; exit 1; fi
  local out code
  set +e; out=$(cd "$B" && ./target/trantor/b3-fs/bin/$2 2>/dev/null); code=$?; set -e
...
./target/release/trantor compose "$B" >/dev/null   # leave the committed default world composed
rm -rf "$B/b3-scratch"
echo "B3 PASS"
