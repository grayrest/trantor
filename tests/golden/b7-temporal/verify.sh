...
set +e; out=$(cd "$B" && ./target/hematite/b7-temporal/bin/b7 2>/dev/null); code=$?; set -e
...
T="$B/target/hematite/b7-temporal/platform/targets/arm64mac"
...
b4syms=$(nm tests/golden/b4-small/target/hematite/b4-small/bin/b4 2>/dev/null | grep -c .)
[[ "$b4syms" -gt 100 ]] || { echo "FAIL: b4 has $b4syms symbols (stripped?) — the temporal_rs check would be vacuous"; exit 1; }
if nm tests/golden/b4-small/target/hematite/b4-small/bin/b4 2>/dev/null | grep temporal_rs >/dev/null; then echo "FAIL: temporal-less world (b4) links temporal_rs"; exit 1; fi
