...
breaks type-body-orphan 'printf "Helper :: [].{\n\tx : I64\n\tx = 1\n\n\texpect x == 2\n}\n" > components/greet-lib/Helper.roc' \
	"Helper is not a module any component exports"
breaks readme-machine-chain 'printf "\n\`\`\`roc\nimport pf.Path\n\nPath.unix(\"/usr\").exists!()?   # True\n\`\`\`\n" >> README.md' \
	"reads the clock or the machine"
...
echo "ok: 25 broken packages each fail naming the break — a baseline module claimed (via dev-deps and via deps), an expect in an unexported module, an expect in no module, unreached expects, a stray world.toml, a README value, a README app, a README near-miss value, a README app's stated output (directly after it and after prose), a stated Bool, a machine read through a chain, a README app's exit status, an expected line, a failing script, an empty cargo suite, a hung script, a malformed timeout, an orphan expect behind a symlink, an orphan expect(...), an orphan expect in a type module's body, a module from two sources, an empty suite, a two-kind suite"
...
passes_within unrelated-text 240 'printf "\n## Installing\n\n\`\`\`text\ntrantor add org/greet\n\`\`\`\n\nNote below a binding:\n\n\`\`\`roc\nloud = Greet.hello(who)\n# TODO(grayrest): greet by name\nloud   # \"hello world\"\n\`\`\`\n" >> README.md'
echo "ok: a symlink loop, a script leaving a child running, a hidden tests/ directory, and a README with an unrelated text block and a # TODO each pass without hanging"
...
variant inner 'rm -rf README.md tests/hello; printf "trap \"sleep 2; touch \\\"%s/cleaned\\\"; exit 0\" TERM\necho \$\$ > \"%s/inner.pid\"\nwhile :; do sleep 0.2; done\n" "$T" "$T" >> tests/script/test.sh'
variant outer "rm -rf README.md tests/hello; printf '\"\$TRANTOR\" test \"%s/inner\"\n' \"$T\" >> tests/script/test.sh"
