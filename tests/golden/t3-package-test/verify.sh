...
# No app suite to build, so only the script can reach the deadline.
breaks script-hangs 'rm -rf tests/hello; printf "sleep 600\n" >> tests/script/test.sh' \
...
echo "ok: 17 broken packages each fail naming the break — a baseline module claimed (via dev-deps and via deps), an expect in an unexported module, an expect in no module, unreached expects, a stray world.toml, an expected line, a failing script, an empty cargo suite, a hung script, a malformed timeout, an orphan expect behind a symlink, an orphan expect(...), an orphan expect in a type module's body, a module from two sources, an empty suite, a two-kind suite"
...
# README examples are not trantor test's to check (D-T3-21): a README stating
# a wrong value is the package's own business.
passes_within readme-unchecked 240 'perl -pi -e "s/# \"hello world\"/# \"hello there\"/" README.md'
! grep -q "README" "$T/readme-unchecked.out" || { echo "FAIL: trantor test still reads README.md"; cat "$T/readme-unchecked.out"; exit 1; }
echo "ok: a symlink loop, a script leaving a child running, a hidden tests/ directory, and a README stating a wrong value each pass without hanging"
...
variant interrupted 'rm -rf tests/hello; printf "echo \$\$ > \"%s/sleeper.pid\"\nexec sleep 600\n" "$T" >> tests/script/test.sh'
...
# One interrupt to an outer run gives a nested `trantor test` its grace: the
# inner suite's TERM handler finishes. A second SIGTERM from the outer runner
# used to reach the nested one as "interrupt again" and kill it at once.
variant inner 'rm -rf tests/hello; printf "trap \"sleep 2; touch \\\"%s/cleaned\\\"; exit 0\" TERM\necho \$\$ > \"%s/inner.pid\"\nwhile :; do sleep 0.2; done\n" "$T" "$T" >> tests/script/test.sh'
variant outer "rm -rf tests/hello; printf '\"\$TRANTOR\" test \"%s/inner\"\n' \"$T\" >> tests/script/test.sh"
rm -f "$T/inner.pid" "$T/cleaned"
set -m
"$TR" test "$T/outer" > "$T/outer.out" 2>&1 & runner=$!
set +m
for _ in $(seq 1800); do [[ -s "$T/inner.pid" ]] && break; sleep 0.1; done
[[ -s "$T/inner.pid" ]] || { kill -KILL "$runner" 2>/dev/null; echo "FAIL: the nested run never reached its script"; tail -20 "$T/outer.out"; exit 1; }
kill -TERM "$runner"; wait "$runner" 2>/dev/null || true
[[ -f "$T/cleaned" ]] || { echo "FAIL: a nested run's suite was killed before its TERM handler finished"; tail -10 "$T/outer.out"; exit 1; }
echo "ok: one interrupt gives a nested run's suite its grace"
echo "T3 PASS"
