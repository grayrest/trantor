...
use trantor_abi::{FsFileReadResult, RocStr};

unsafe extern "C-unwind" {
    fn trantor__capstdfs__file_read(path: RocStr) -> FsFileReadResult;
}

#[unsafe(no_mangle)]
pub extern "C-unwind" fn trantor__audit__file_read(path: RocStr) -> FsFileReadResult {
    ...
    unsafe { trantor__capstdfs__file_read(path) }
}
