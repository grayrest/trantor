...
# mechanisms proven on trantor's pinned compiler, before any DB.
...
if ! _b=$(./target/release/trantor build "$S" --app app --out sq0 2>&1); then echo "FAIL: build sq0" >&2; echo "$_b" >&2; exit 1; fi
# the glue generated the erased-callable ABI (apply_i64! takes a Box(closure)).
grep -q 'RocErasedCallableFn' "$S/target/trantor/sq0-substrate/abi/src/generated.rs" || { echo "FAIL: no erased-callable ABI generated"; exit 1; }

set +e; out=$(cd "$S" && ./target/trantor/sq0-substrate/bin/sq0 2>/dev/null); code=$?; set -e
...
g=$(cd "$S" && TRANTOR_ALLOC_GAUGE=1 ./target/trantor/sq0-substrate/bin/sq0 2>&1 1>/dev/null | grep '^\[alloc-gauge\]' || true)
