...
Stdout :: [].{
	...
	line! = |message| widen_stdout_err(Stdio.stdout_line!(message))
	...
	write! = |message| widen_stdout_err(Stdio.stdout_write!(message))
	...
	write_bytes! = |bytes| widen_stdout_err(Stdio.stdout_write_bytes!(bytes))
}
