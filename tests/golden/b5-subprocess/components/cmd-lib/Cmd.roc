...
Cmd :: {
	...
	exec_output_inherit_stdin! = |cmd| {
		cmd_str = to_str(cmd)
		exec_try = Host.cmd_exec_output_inherit_stdin!(to_host_cmd(cmd))
	...
	## signal and cannot be told from a genuine exit code of 143. Real exit codes
	## are 0..=255, so a negative result here is unambiguous.
	##
	## Off unix a signal death is still an error, since there is no signal number
	## to report.
	##
	## ```roc
	...
	exec_status! = |cmd| {
		...
		match Host.cmd_exec_status!(to_host_cmd(cmd)) {
			Ok(num) => Ok(num)
			Err(io_err) => Err(FailedToGetExitCode({ command, err: io_err }))
		}
	}
...
## Migration bridge (R8): seahaven's Env.var! is OsStr-typed; this platform's is
## Str-typed. Re-wrap so the PATH-search code above is otherwise verbatim.
env_var_os! : Str => Try(OsStr, [VarNotFound(OsStr), EnvErr(IOErr), ..])
env_var_os! = |name| {
	match Env.var!(name) {
		Ok(v) => Ok(OsStr.from_str(v))
		Err(VarNotFound(n)) => Err(VarNotFound(OsStr.from_str(n)))
		Err(EnvErr(e)) => Err(EnvErr(e))
		Err(_) => Err(VarNotFound(OsStr.from_str(name)))
	}
}
