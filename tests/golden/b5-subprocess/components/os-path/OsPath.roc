...
OsPath := [Text(Str), Raw(List(U8))].{
	...
	utf8 : Str -> OsPath
	utf8 = |s| from_str(s)
	unix_bytes : List(U8) -> OsPath
	unix_bytes = |b| OsPath.Raw(b)
	windows_u16s : List(U16) -> OsPath
	windows_u16s = |_u| from_str("")
	is_executable! : OsPath => Try(Bool, IOErr)
	is_executable! = |p| {
	...
	is_eq : OsPath, OsPath -> Bool
	is_eq = |a, b| to_bytes(a) == to_bytes(b)
	to_hash : OsPath, Hasher -> Hasher
