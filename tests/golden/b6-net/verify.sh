...
if ! _b=$(./target/release/trantor build "$B" --app app --out b6 2>&1); then echo "FAIL: build b6" >&2; echo "$_b" >&2; exit 1; fi
set +e; out=$(cd "$B" && ./target/trantor/b6-net/bin/b6 2>/dev/null); code=$?; set -e
...
grep -q 'ureq' "$B/target/trantor/b6-net/components/http-host/Cargo.toml" || { echo "FAIL: http-host is not built over ureq"; exit 1; }
grep -q 'body_stream' "$B/interfaces/sync-http/HttpHost.roc" || { echo "FAIL: HttpHost.Response is not a streaming body"; exit 1; }
if ! _b=$(./target/release/trantor build "$B" --app http-stream-app --out b6-httpstream 2>&1); then echo "FAIL: build http-stream-app" >&2; echo "$_b" >&2; exit 1; fi
set +e; sout=$(cd "$B" && ./target/trantor/b6-net/bin/b6-httpstream 2>/dev/null); scode=$?; set -e
...
# H16: TRANTOR_HTTP_MAX_REDIRECTS=0 returns the 3xx unfollowed.
rout=$(cd "$B" && TRANTOR_HTTP_MAX_REDIRECTS=0 ./target/trantor/b6-net/bin/b6-httpstream 2>/dev/null | grep '^redirect:' || true)
[[ "$rout" == "redirect: 302 0" ]] || { echo "FAIL: TRANTOR_HTTP_MAX_REDIRECTS=0 should return the 302 (got: $rout)"; exit 1; }
echo "ok: TRANTOR_HTTP_MAX_REDIRECTS=0 returns the 302 unfollowed"

# alloc-gauge: the streaming body paths drop-balance (heap-data leaks resource
# live() can't see).
g=$(cd "$B" && TRANTOR_ALLOC_GAUGE=1 ./target/trantor/b6-net/bin/b6-httpstream 2>&1 1>/dev/null | grep '^\[alloc-gauge\]' || true)
...
# via TRANTOR_HTTP_EXTRA_CA (additive over webpki-roots, H13), and run an https
...
./target/release/trantor compose "$B" >/dev/null
grep -q '^default = \["tls"\]' "$B/target/trantor/b6-net/components/http-host/Cargo.toml" || { echo "FAIL: default world should compose http-host with tls"; exit 1; }
if ! _b=$(./target/release/trantor build "$B" --app app --out b6 2>&1); then echo "FAIL: build b6" >&2; echo "$_b" >&2; exit 1; fi
cap "$ROC" build --output="$B/target/trantor/b6-net/bin/https-app" "$B/https-app/main.roc" >/dev/null 2>&1 || { echo "FAIL: build https-app"; exit 1; }
tls_on=$({ ar t "$B/target/trantor/b6-net/platform/targets/arm64mac/libhttp_host.a" 2>/dev/null || true; } | grep -icE 'rustls|webpki' || true)
...
ho=$(cd "$B" && TRANTOR_TEST_CERT="$CERT" TRANTOR_HTTP_EXTRA_CA="$CERT" ./target/trantor/b6-net/bin/https-app 2>/dev/null || true)
...
hu=$(cd "$B" && TRANTOR_TEST_CERT="$CERT" ./target/trantor/b6-net/bin/https-app 2>/dev/null || true)
[[ "$hu" != "https: 200 https-hello" && "$hu" == https:* ]] || { echo "FAIL: untrusted cert should be rejected (got: $hu)"; exit 1; }
echo "ok: without TRANTOR_HTTP_EXTRA_CA the cert is rejected — trust is additive/opt-in, not blanket"

g=$(cd "$B" && TRANTOR_TEST_CERT="$CERT" TRANTOR_HTTP_EXTRA_CA="$CERT" TRANTOR_ALLOC_GAUGE=1 ./target/trantor/b6-net/bin/https-app 2>&1 1>/dev/null | grep '^\[alloc-gauge\]' || true)
...
./target/release/trantor compose "$B" --world world-notls.toml >/dev/null
grep -q '^default = \[\]' "$B/target/trantor/b6-net/components/http-host/Cargo.toml" || { echo "FAIL: world-notls should compose http-host with tls off"; exit 1; }
if ! _b=$(./target/release/trantor build "$B" --world world-notls.toml --app app --out b6 2>&1); then echo "FAIL: build b6 (tls off)" >&2; echo "$_b" >&2; exit 1; fi
cap "$ROC" build --output="$B/target/trantor/b6-net/bin/https-app" "$B/https-app/main.roc" >/dev/null 2>&1 || { echo "FAIL: build https-app (tls off)"; exit 1; }
hn=$(cd "$B" && TRANTOR_TEST_CERT="$CERT" TRANTOR_HTTP_EXTRA_CA="$CERT" ./target/trantor/b6-net/bin/https-app 2>/dev/null || true)
[[ "$hn" == "https: other" ]] || { echo "FAIL: tls-off world should reject https with Other (got: $hn)"; exit 1; }
tls_off=$({ ar t "$B/target/trantor/b6-net/platform/targets/arm64mac/libhttp_host.a" 2>/dev/null || true; } | grep -icE 'rustls|webpki' || true)
...
b4syms=$({ nm tests/golden/b4-small/target/trantor/b4-small/bin/b4 2>/dev/null || true; } | grep -c . || true)
[[ "$b4syms" -gt 100 ]] || { echo "FAIL: b4 has $b4syms symbols (stripped?) — the check would be vacuous"; exit 1; }
b4net=$({ nm tests/golden/b4-small/target/trantor/b4-small/bin/b4 2>/dev/null || true; } | grep -icE 'ureq|rustls' || true)
...
if ! _b=$(./target/release/trantor build "$B" --app app --out b6 2>&1); then echo "FAIL: build b6 (default)" >&2; echo "$_b" >&2; exit 1; fi
echo "HC4 PASS"
