# hematite dev recipes.

# Generate a self-signed localhost cert for HEMATITE_HTTP_EXTRA_CA (H13/H14).
# Writes the cert PEM to <cert-path> and the key to <cert-path>.key, and prints
# the `export HEMATITE_HTTP_EXTRA_CA=<cert-path>` line to wire it into the HTTP
# client. local-cert is a standalone crate (its crypto deps stay out of the
# hematite tool's build), so it's run by --manifest-path, not `-p`.
