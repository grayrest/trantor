...
Gate H1b of [`plans/2026-09-04-hematite-v1.md`](../plans/2026-09-04-hematite-v1.md),
...
**So D18-C is viable:** hematite can splice the driver's `requires` verbatim as
...
This is the platform-im comment reproduced exactly. **Hard rule for hematite:
run `roc check` on the composed platform before `roc build`.** `check` turns the
compiler's silent segfault into a precise diagnostic. Equivalently, hematite can
...
**Hard rule for hematite (folds into D13):** a binding module must **re-export
...
renamed per D14, confirm hematite rewrites `[requires.uses]` to the renamed
module) is a hematite-behavior test, not a compiler-behavior one. At the Roc
